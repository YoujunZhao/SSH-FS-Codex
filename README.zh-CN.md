...
SSH FS Codex 是一个 VS Code 插件，用来让 Codex CLI 或 VS Code Codex 插件可以处理 SSH FS 里的服务器文件。它会把 SSH FS 远程目录同步到一个真实的本地 mirror 文件夹，让 Codex 在本地路径里工作；你确认修改后，再手动同步回 SSH FS 远程目录。
...
4. 运行 `Codex SSH FS Bridge: Prepare Workspace for Codex`。
5. 插件会把远程目录同步到本地 mirror，并自动打开生成的本地 `.code-workspace`。
6. 保持原来的 SSH FS 窗口连接服务器，同时在新打开的本地 mirror 窗口里使用 VS Code Codex 插件。
7. 如果你用的是 Codex CLI，也可以在本地 mirror 窗口的 VS Code terminal 里运行：

```bash
codex
```

8. 让 Codex 修改本地 mirror 里的文件。此时 Codex 能读取 mirror workspace 中的文件，并能拿到这个本地 VS Code workspace/editor context。
...
- `Codex SSH FS Bridge: Prepare Workspace for Codex`  
  推荐入口。从 SSH FS 远程目录同步到本地 mirror，并直接打开可供 VS Code Codex 插件使用的本地 workspace。

- `Codex SSH FS Bridge: Sync Workspace to Local Mirror`  
  从 SSH FS 远程目录拉取到本地 mirror。你在远程 workspace 窗口或 mirror 窗口都可以用它刷新 mirror；默认也会打开本地 mirror workspace。
...
- Codex 插件读取的是本地 mirror，不是直接读取 `ssh://...`、`sshfs://...` 或 `sftp://...` URI。这是为了让 Codex 获得普通本地路径和 IDE context。
